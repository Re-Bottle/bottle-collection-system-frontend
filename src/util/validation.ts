...
export const nameValidation = (firstName: string, lastName: string): validationResult => {
...
export const passwordValidation = (password: string): validationResult => {
    if (!(password !== undefined && passwordRegex.test(password))) {
        return {
            message: "Password must be at least 6 characters long, include an uppercase letter, a number, and a special character.",
            result: false
        }
    }
    return {
